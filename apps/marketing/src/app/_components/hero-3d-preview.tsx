...
export function Hero3DPreview() {
  ...
  useEffect(() => {
    ...
    // Multiplier dá folga pra mar aparecer em volta sem afastar demais
    ...
    // Group raiz (drift sutil) > boatBobGroup (bob/roll/pitch surf)
    // > boatScaleGroup (BOAT_SCALE + rotação 180° pra mostrar proa).
    ...
    // Coord swap deixa proa do Benchy em -Z; câmera +Z olhando origem
    // vê a popa. 180° em Y traz a proa pra frente da câmera.
    ...
    // Ring no boatBobGroup (FORA do scale) — fica do tamanho do footprint
    // em world coords, alinhado com a base do mar.
    ...
    // Energy/pulse/halo ficam no rootGroup mas FORA do boatBobGroup —
    // não querem oscilar Y com o barco. Raio acompanha o barco escalado.
    ...
    // Floor escuro embaixo do barco — gradient simulado via círculo
    // com opacity alta no centro, esmaecendo nas bordas.
    ...
    // Aplica progress fixo em 50% — metade impressa, metade ghost.
    // Mesh em world = meshHeight * BOAT_SCALE (printed/ghost dentro do
    // boatScaleGroup). Clip planes são world space, então clipY tem
    // que estar nessa escala.
    ...
    const tick = (now: number): void => {
      ...
      // Drift sutil — direção visível das ondas é preservada
      rootGroup.rotation.y += dt * 0.04;
      // Texture scrolling (data-stream subindo nos cilindros)
      energyTexture.offset.y -= dt * 0.4;
      pulseTexture.offset.y -= dt * 0.85;

      // Pulse de respiração 2.4s nos cilindros
