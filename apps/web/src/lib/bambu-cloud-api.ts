/**
 * Cliente HTTP server-side pra Bambu Cloud (não-oficial). Usado pelos
 * Route Handlers do Next.js em /app/api/bambu/*.
 *
 * Espelhado de apps/api/src/services/bambu-cloud.ts — TODO extrair pra
 * um pacote @printstudio/bambu-cloud-api compartilhado.
 */
...
export async function loginWithCode(email: string, code: string): Promise<BambuLoginResult> {
  ...
  if (!res.ok) {
    ...
    if (res.status === 400 && body.includes('error')) {
      throw new BambuCloudError('BAMBU_INVALID_CODE', 'Código inválido ou expirado', res.status);
    }
  ...
  if (data.loginType === 'verifyCode') {
    throw new BambuCloudError('BAMBU_VERIFY_REQUIRED', 'Bambu pediu nova verificação', res.status);
  }
  if (data.loginType === 'tfa') {
    throw new BambuCloudError('BAMBU_TFA_REQUIRED', 'Conta exige 2FA — não suportado', res.status);
  }
  if (!data.accessToken) {
    throw new BambuCloudError(
      'BAMBU_UPSTREAM_ERROR',
  ...
  return {
    ...
    expiresIn: data.expiresIn ?? 60 * 60 * 24 * 90,
  };
}
...
export async function listDevices(accessToken: string): Promise<BambuDeviceRaw[]> {
  ...
  if (!res.ok) throwUpstream(res.status, await res.text());
  const data = (await res.json()) as { devices?: BambuDeviceRaw[] };
  return data.devices ?? [];
}
...
export async function getUserId(accessToken: string): Promise<string> {
  ...
  if (!res.ok) throwUpstream(res.status, await res.text());
  const data = (await res.json()) as { uidStr?: string; uid?: number | string };
  const uid = data.uidStr ?? (data.uid !== undefined ? String(data.uid) : null);
  if (!uid) {
    throw new BambuCloudError('BAMBU_UPSTREAM_ERROR', 'Bambu preference sem uid', res.status);
  }
  return uid;
}
