...
/**
 * Variante kiosk do PrinterCard — tipografia escalável via clamp(),
 * status hierárquico (cor de fundo muda conforme severidade) e
 * sem hover (kiosk é touch ou monitor parado).
 */
export function KioskPrinterCard({ printerId, name, state }: Props) {
  ...
  return (
    ...
    >
      {/* Status header — barra colorida no topo */}
      <div className={cn('px-4 py-2 flex items-center gap-2', TONE_HEADER_CLASSES[tone])}>
        ...
        {hmsCount > 0 ? (
          <span
            className="ml-auto rounded-full bg-danger px-2 py-0.5 font-bold text-danger-foreground"
      ...
      <div className="p-5 space-y-4">
        ...
        {/* Progresso GIANT quando imprimindo, ou stage fallback */}
        {printing ? (
          <>
            ...
            {hasError
              ...
              : status === 'FINISH'
                ...
                : status === 'OFFLINE'
                  ...
                  : activeSlot?.filamentType ?? 'Pronta para imprimir'}
          </div>
        )}
